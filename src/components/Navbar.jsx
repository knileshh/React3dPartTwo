  ...
  return (
    ...
    <div className='w-full flex justify-between items-center max-w-7xl mx-auto'>
      <Link to="/" className='flex items-center gap-2' onClick={() => {
        ...
        <p className='text-white text-[18px] font-bold cursor-pointer flex'> Nilesh's &nbsp;<span className='sm:block hidden'>Portfolio</span></p>
      ...
      <div className='sm:hidden flex flex-1 justify-end items-center'>
        ...
        {navLinks.map((link) => (
          <li key={link.id} className={`${active === link.title ? "text-white" : "text-secondary"} font-poppins font-medium cursor-pointer`} 
            onClick={() => {
            setToggle(!toggle)
